...
Çoğu zaman geliştiriciler karakter dokularını dörtlü üzerinde oluşturmayı seçer. Bu dokulu dörtlüyü tek başına oluşturmak çok zor değildir, ancak ilgili karakterleri bir dokuya sokmak zor olabilir. Bu eğitselde, FreeType kütüphanesini kullanarak metin oluşturmak için daha gelişmiş fakat esnek bir teknik uygulayacağız. 

## Klasik Metin Sahneleme: bitmap Yazı Tipleri
Eskiden metin oluşturma işleminde, uygulamanız için bir font seçmek (veya kendiniz bir tane oluşturmak) ve hepsini tek bir büyük dokuya yapıştırmak için bu fonttan tüm ilgili karakterleri çıkarmak yer alıyordu. Şu andan itibaren bir bitmap fontu olarak adlandırdığımız böyle bir doku, dokunun önceden tanımlanmış bölgelerinde kullanmak istediğimiz tüm karakter sembollerini içerir. Yazı tipinin bu karakter sembolleri glifler(ing. glyphs) olarak adlandırılır. Her glif, kendisiyle ilişkilendirilmiş belirli bir doku koordinat bölgesine sahiptir. Bir karakter oluşturmak istediğinizde, bitmap fontunun bu bölümünü bir 2-boyutlu dörtlüye dönüştürerek karşılık gelen glifi seçersiniz.
